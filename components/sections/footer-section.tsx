...
export default function FooterSection({ t }: FooterSectionProps) {
  return (
    <footer className="w-full pt-20 pb-10 relative" id="footer">
      {/* Footer grid */}
      <div className="relative z-10 grid grid-cols-2 md:grid-cols-4 gap-8 pb-10 border-t border-muted-foreground/10">
        {/* Brand */}
          ...
          <div className="flex items-center gap-3 mt-5">
            {socialLinks.map(({ icon: Icon, href, label }) => (
              <a
                ...
                href={href}
                aria-label={label}
                className="w-9 h-9 rounded-lg flex items-center justify-center transition-all duration-200 hover:scale-110 bg-card/80 border border-muted-foreground/10"
              >
      ...
      {/* Bottom bar */}
      <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-4 pt-6 border-t border-muted-foreground/10">
